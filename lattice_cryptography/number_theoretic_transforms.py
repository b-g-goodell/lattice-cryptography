...
def _ntt_coefs_to_splitcoefs(val: Coefs, n: int) -> SplitCoefs:
    """
    ...
    deg_mod_of_ntt: int = 2 * val.deg_mod // n
    rou: int
    rou_inv: int
    ...
    while len(padvals) < 2 * deg_mod:
        padvals += [0]  # Pad with enough zeros to apply the NTT
    split: list[list[int]] = _split_into_sublists_of_len_k(val=padvals, k=n)  # split the list into sublists for applying the NTT
    ntts: list[list[int]] = [_ntt_base(mod=coef_mod, n=n, vals=val, inverse=False) for val in split]  # apply the NTTs
...
def _intt_splitcoefs_to_coefs(val: SplitCoefs, n: int) -> Coefs:
    """

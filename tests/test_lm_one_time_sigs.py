...
from lattice_cryptography.lm_one_time_sigs import PublicParameters, Message, Challenge, Signature, OneTimeKeyTuple, \
    make_setup_parameters, make_one_key, keygen, sign, verify, LPs, SALTs, BDs, WTs, DISTRIBUTION, make_random_seed
...
@pytest.mark.parametrize(
    "secpar,sp,pp,j,seed,secret_seed,sk_bd,sk_wt,left_sk,right_sk,sk,left_vk,right_vk,vk,ch_bd,ch_wt,msg,sig_ch",
    MAKE_SIGNATURE_CHALLENGE_CASES)
def test_correct_specific(secpar, sp, pp, j, seed, secret_seed, sk_bd, sk_wt, left_sk, right_sk, sk, left_vk, right_vk,
                          vk, ch_bd, ch_wt, msg, sig_ch):
    sig = sign(pp=pp, otk=(secret_seed, sk, vk), msg=msg)
    assert verify(pp=pp, otvk=vk, msg=msg, sig=sig)


@pytest.mark.parametrize(
    "secpar,sp,pp,j,seed,secret_seed,sk_bd,sk_wt,left_sk,right_sk,sk,left_vk,right_vk,vk,ch_bd,ch_wt,msg,sig_ch,sig_a,valid_a,sig_b,valid_b",
    MAKE_SIGN_CASES)
def test_verify(mocker, secpar, sp, pp, j, seed, secret_seed, sk_bd, sk_wt, left_sk, right_sk, sk, left_vk, right_vk,
                vk, ch_bd, ch_wt, msg, sig_ch, sig_a, valid_a, sig_b, valid_b):

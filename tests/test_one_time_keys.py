...
# Prepare stuff for parameterization.
SAMPLE_SIZE: int = 2 ** 0
MIN_LOG2D: int = 5
MAX_LOG2D: int = 7
...
def test_one_time_signing_key_eq(secpar, lp, seed, secret_seed, wit_bd, wit_wt, wit, key_ch, stat, sk_bd, sk_wt, left_sk, right_sk, sk):
    x = OneTimeSigningKey(secpar=secpar, lp=lp, left_key=left_sk, right_key=right_sk, const_time_flag=False)
    assert x == sk
...
def test_make_one_key(mocker, secpar, sp, pp: Dict[str, Any], j, seed, secret_seed, sk_bd, sk_wt, left_sk, right_sk, sk, left_vk, right_vk, vk):
    ...
    observed_key_tuple = make_one_key(pp=pp, const_time_flag=False)
    ...
    assert not observed_sk[0].const_time_flag
    assert not observed_sk[1].const_time_flag
    ...
    new_left_sk = deepcopy(left_sk)
    new_right_sk = deepcopy(right_sk)
    new_left_sk.const_time_flag = True
    new_right_sk.const_time_flag = True
    mocker.patch('lattice_cryptography.one_time_keys.make_random_seed', return_value=secret_seed)
    mocker.patch('lattice_cryptography.one_time_keys.hash2polynomialvector', side_effect=[new_left_sk, new_right_sk])
    observed_key_tuple = make_one_key(pp=pp, const_time_flag=True)
    observed_seed, observed_sk, observed_vk = observed_key_tuple
    assert observed_seed == secret_seed
    assert observed_sk == sk
    assert sk[0] == left_sk
...
def test_sign_core(mocker, secpar, sp, pp, j, seed, secret_seed, sk_bd, sk_wt, left_sk, right_sk, sk, left_vk, right_vk, vk, ch_bd, ch_wt, msg, sig_ch, expected_sig_a, expected_valid_a, expected_sig_b, expected_valid_b):
    sig_ch_matches_left_key = sig_ch.const_time_flag == left_sk.const_time_flag
    sig_ch_matches_right_key = sig_ch.const_time_flag == right_sk.const_time_flag
    left_key_matches_right_key = left_sk.const_time_flag == right_sk.const_time_flag
    if not all([sig_ch_matches_right_key, sig_ch_matches_left_key, left_key_matches_right_key]):
        print()
    assert sig_ch_matches_left_key and sig_ch_matches_right_key and left_key_matches_right_key
...
def test_verify_core(mocker, secpar, sp, pp, j, seed, secret_seed, sk_bd, sk_wt, left_sk, right_sk, sk, left_vk, right_vk, vk, ch_bd, ch_wt, msg, sig_ch, sig_a, valid_a, sig_b, valid_b):
    mocker.patch('lattice_cryptography.one_time_keys.challenge_core', return_value=sig_ch)
    assert sk[0].const_time_flag == sk[1].const_time_flag == sig_ch.const_time_flag
    assert sig_a == sig_b

...
COEFS_PARAMS_CASES: list[tuple[int, int, int, int, list[int]]] = [
    (q[0], d[0], n, 1, [randrange(q[0]) for i in range(d[0])]) for q in IS_ODD_PRIME_CASES for d in IS_POS_INT_AND_POW_TWO_CASES for n in [d[0] >> i for i in range(ceil(log2(d[0]))) for j in range(SAMPLE_SIZE)]
] + [
    (q[0], d[0], n, 2, [randrange(q[0]) for i in range(d[0])]) for q in IS_ODD_PRIME_CASES for d in IS_POS_INT_AND_POW_TWO_CASES for n in [d[0] >> i for i in range(ceil(log2(d[0]))) for j in range(SAMPLE_SIZE)]
]
COEFS_CASES: list[tuple[int, int, int, int, list[int], Coefs]] = [
    (i[0], i[1], i[2], i[3], i[4], Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=i[4])) for i in COEFS_PARAMS_CASES
]
...
def test_coefs_init_and_repr_and_eq(coef_mod, max_deg, ord_of_prim_rou, const, vals, initialized_object):
    ...
    assert initialized_object.ord_of_prim_rou == ord_of_prim_rou
    assert initialized_object.half_coef_mod == coef_mod//2
    assert initialized_object.lg_coef_mod == ceil(log2(coef_mod))
    assert initialized_object.lg_ord_of_prim_rou == ceil(log2(ord_of_prim_rou))
    ...
    assert Coefs(coef_mod=coef_mod, deg_mod=max_deg, ord_of_prim_rou=ord_of_prim_rou, const=const, vals=vals) == initialized_object


COEFS_ADD_PREPRECASES: list[tuple[int, int, int, int, list[int], list[int]]] = [
    (q[0], d[0], n, 1, [randrange(q[0]) for i in range(d[0])], [randrange(q[0]) for i in range(d[0])]) for q in IS_ODD_PRIME_CASES for d in IS_POS_INT_AND_POW_TWO_CASES for n in [d[0] >> i for i in range(ceil(log2(d[0]))) for j in range(SAMPLE_SIZE)]
] + [
    (q[0], d[0], n, 2, [randrange(q[0]) for i in range(d[0])], [randrange(q[0]) for i in range(d[0])]) for q in IS_ODD_PRIME_CASES for d in IS_POS_INT_AND_POW_TWO_CASES for n in [d[0] >> i for i in range(ceil(log2(d[0]))) for j in range(SAMPLE_SIZE)]
]
COEFS_ADD_PRECASES: list[tuple[int, int, int, int, list[int], list[int], list[int]]] = [
    (i[0], i[1], i[2], i[3], i[4], i[5], [_reduce(val=x+y, mod=i[0]) for x, y in zip(i[4], i[5])]) for i in COEFS_ADD_PREPRECASES
]
COEFS_ADD_CASES: list[tuple[int, int, int, int, list[int], list[int], list[int], Coefs, Coefs, Coefs, Coefs]] = [
    (i[0], i[1], i[2], i[3], i[4], i[5], i[6], Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=i[4]), Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=i[5]), Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=i[6]), Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=[x + y for x, y in zip(i[4], i[5])])) for i in COEFS_ADD_PRECASES
]
...
COEFS_SUB_CASES: list[tuple[int, int, int, int, list[int], list[int], list[int], Coefs, Coefs, Coefs, Coefs]] = [
    (i[0], i[1], i[2], i[3], i[4], i[5], i[6], Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=i[4]), Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=i[5]), Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=i[6]), Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=[x - y for x, y in zip(i[4], i[5])])) for i in COEFS_SUB_PRECASES
]
...
COEFS_MUL_PRECASES: list[tuple[int, int, int, int, list[int], list[int], list[int], Coefs, Coefs, Coefs, Coefs]] = [
    i + tuple([
        Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=i[4]),
        Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=i[5]),
        Coefs(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], const=i[3], vals=i[6])
...
PTVALS_PREPRECASES: list[tuple[int, int, int, int, list[list[int]]]] = [
    (q[0], d[0], n, _get_nth_prim_rou_and_inv(mod=q[0], n=n)[0], [[_reduce(val=randrange(q[0]), mod=q[0]) for k in range((2*d[0])//n)] for j in range(n)]) for q in IS_ODD_PRIME_CASES for d in IS_POS_INT_AND_POW_TWO_CASES for n in [2**i for i in range(ceil(log2(d[0]))+1, 0, -1)]
]
PTVALS_PRECASES: list[tuple[int, int, int, int, list[list[int]], list[Coefs]]] = [
    i + tuple([
        [Coefs(coef_mod=i[0], deg_mod=len(j) - 1, ord_of_prim_rou=i[2], const=i[3] ** (k + 1), vals=j) for k, j in enumerate(i[-1])]
    ]) for i in PTVALS_PREPRECASES
]
PTVALS_CASES: list[tuple[int, int, int, int, list[list[int]], list[Coefs], PtVals]] = [
    i + tuple([
        PtVals(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], rou=i[3], vals=i[-1])
    ]) for i in PTVALS_PRECASES
]
...
def test_ptvals_init_and_repr_and_eq(coef_mod, max_deg, ord_of_prim_rou, const, coefs, vals, initialized_object):
    assert initialized_object == PtVals(coef_mod=coef_mod, deg_mod=max_deg, ord_of_prim_rou=ord_of_prim_rou, rou=const, vals=vals)


PTVALS_ADD_PREPREPRECASES: list[tuple[int, int, int, int, list[list[int]], list[list[int]]]] = [
    (q[0], d[0], n, _get_nth_prim_rou_and_inv(mod=q[0], n=n)[0], [[_reduce(val=randrange(q[0]), mod=q[0]) for k in range((2*d[0])//n)] for j in range(n)], [[_reduce(val=randrange(q[0]), mod=q[0]) for k in range((2*d[0])//n)] for j in range(n)]) for q in IS_ODD_PRIME_CASES for d in IS_POS_INT_AND_POW_TWO_CASES for n in [2**i for i in range(ceil(log2(d[0]))+1, 0, -1)]
]
PTVALS_ADD_PREPRECASES: list[tuple[int, int, int, int, list[list[int]], list[list[int]], list[list[int]], list[Coefs], list[Coefs]]] = [
    i + tuple([
        [[_reduce(val=x+y, mod=i[0]) for x, y in zip(a, b)] for a, b in zip(i[-1], i[-2])],
        [Coefs(coef_mod=i[0], deg_mod=len(j) - 1, ord_of_prim_rou=i[2], const=i[3] ** (k + 1), vals=j) for k, j in enumerate(i[-2])],
        [Coefs(coef_mod=i[0], deg_mod=len(j) - 1, ord_of_prim_rou=i[2], const=i[3] ** (k + 1), vals=j) for k, j in enumerate(i[-1])]
    ]) for i in PTVALS_ADD_PREPREPRECASES
]
PTVALS_ADD_PRECASES: list[tuple[int, int, int, int, list[list[int]], list[list[int]], list[list[int]], list[Coefs], list[Coefs], list[Coefs], PtVals, PtVals]] = [
    i + tuple([
        [Coefs(coef_mod=i[0], deg_mod=len(j) - 1, ord_of_prim_rou=i[2], const=i[3] ** (k + 1), vals=j) for k, j in enumerate(i[-3])],
        PtVals(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], rou=i[3], vals=i[-2]),
        PtVals(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], rou=i[3], vals=i[-1])
    ]) for i in PTVALS_ADD_PREPRECASES
]
PTVALS_ADD_CASES: list[tuple[int, int, int, list[list[int]], list[list[int]], list[list[int]], list[Coefs], list[Coefs], list[Coefs], PtVals, PtVals, PtVals, PtVals]] = [
    i + tuple([
        PtVals(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], rou=i[3], vals=i[-3]),
        i[-1] + i[-2]
    ]) for i in PTVALS_ADD_PRECASES
]


@pytest.mark.parametrize("coef_mod,max_deg,ord_of_prim_rou,const,coefs_a,coefs_b,coefs_a_plus_b,vals_a,vals_b,vals_a_plus_b,a,b,c,a_plus_b", PTVALS_ADD_CASES)
def test_ptvals_add(coef_mod, max_deg, ord_of_prim_rou, const, coefs_a, coefs_b, coefs_a_plus_b, vals_a, vals_b, vals_a_plus_b, a, b, c, a_plus_b):
    assert a + b == a_plus_b == b + a == c


PTVALS_MUL_PREPREPRECASES: list[tuple[int, int, int, int, list[list[int]], list[list[int]], list[list[int]]]] = [
    # (17, 4, 8, _get_nth_prim_rou_and_inv(mod=17, n=8)[0], [[1], [2], [3], [4], [5], [6], [7], [8]], [[7], [8], [-8], [-7], [-6], [-5], [-4], [-3]], [[7], [-1], [-7], [6], [4], [4], [6], [-7]]),
    (17, 4, 4, _get_nth_prim_rou_and_inv(mod=17, n=4)[0], [[1, 2], [3, 4], [5, 6], [7, 8]], [[7, 8], [-8, -7], [-6, -5], [-4, -3]], [[-6, 5], [-1, -2], [3, 7], [-4, -2]]),
]
PTVALS_MUL_PREPRECASES: list[tuple[int, int, int, int, list[list[int]], list[list[int]], list[list[int]], list[Coefs], list[Coefs], list[Coefs]]] = [
    i + tuple([
        [Coefs(coef_mod=i[0], deg_mod=(2 * i[1]) // i[2], ord_of_prim_rou=i[2], const=i[3] ** (k + 1), vals=vals) for k,vals in enumerate(i[4])],
        [Coefs(coef_mod=i[0], deg_mod=(2 * i[1]) // i[2], ord_of_prim_rou=i[2], const=i[3] ** (k + 1), vals=vals) for k,vals in enumerate(i[5])],
        [Coefs(coef_mod=i[0], deg_mod=(2 * i[1]) // i[2], ord_of_prim_rou=i[2], const=i[3] ** (k + 1), vals=vals) for k,vals in enumerate(i[6])]
    ]) for i in PTVALS_MUL_PREPREPRECASES
]
PTVALS_MUL_PRECASES: list[tuple[int, int, int, int, list[list[int]], list[list[int]], list[list[int]], list[Coefs], list[Coefs], list[Coefs], PtVals, PtVals, PtVals]] = [
    i + tuple([
        PtVals(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], rou=i[3], vals=i[7]),
        PtVals(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], rou=i[3], vals=i[8]),
        PtVals(coef_mod=i[0], deg_mod=i[1], ord_of_prim_rou=i[2], rou=i[3], vals=i[9]),
    ]) for i in PTVALS_MUL_PREPRECASES
]
PTVALS_MUL_CASES: list[tuple[int, int, int, int, list[list[int]], list[list[int]], list[list[int]], list[Coefs], list[Coefs], list[Coefs], PtVals, PtVals, PtVals, PtVals]] = [
    i + tuple([
        i[-2]*i[-3]
...
@pytest.mark.parametrize("coef_mod,max_deg,ord_of_prim_rou,rou,coefs_a,coefs_b,coefs_c,vals_a,vals_b,vals_c,a,b,c,a_times_b,a_times_b_vals", PTVALS_MUL_CASES)
def test_ptvals_mul(coef_mod, max_deg, ord_of_prim_rou, rou, coefs_a, coefs_b, coefs_c, vals_a, vals_b, vals_c, a, b, c, a_times_b, a_times_b_vals):
    assert a*b == c
    assert b*a == c
    assert a_times_b == c
...
def test_coefs_mul_matches_ntt_mul():
    ...
    one_poly: Coefs = Coefs(coef_mod=coef_mod, deg_mod=deg_mod, ord_of_prim_rou=ord_of_prim_rou, const=const, vals=one_vals)

    some_vals: list[int] = [randrange(coef_mod) for _ in range(deg_mod)]
    some_poly: Coefs = Coefs(coef_mod=coef_mod, deg_mod=deg_mod, ord_of_prim_rou=ord_of_prim_rou, const=const, vals=some_vals)

    assert one_poly * some_poly == some_poly
    ...
    some_ntt: PtVals = ntt(val=some_poly, ord_of_prim_rou=ord_of_prim_rou)

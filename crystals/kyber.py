...
ENCODED_CPA_PKE_SK_LEN: dict[int, int] = {security_parameter: 12 * PARAMS[security_parameter]['k'] * DEGREE / 8 for
                                          security_parameter in ALLOWABLE_SECURITY_PARAMETERS}
ENCODED_CPA_PKE_PK_LEN: dict[int, int] = {security_parameter: 12 * PARAMS[security_parameter]['k'] * DEGREE / 8 + 32 for
                                          security_parameter in ALLOWABLE_SECURITY_PARAMETERS}
ENCODED_CPA_PKE_CIPHERTEXT_LEN: dict[int, int] = {security_parameter: (PARAMS[security_parameter]['d_u'] *
                                                                       PARAMS[security_parameter]['k'] +
                                                                       PARAMS[security_parameter]['d_v']) * DEGREE / 8
                                                  for security_parameter in ALLOWABLE_SECURITY_PARAMETERS}
ENCODED_CCA_KEM_SK_LEN: dict[int, int] = {security_parameter: 24 * PARAMS[security_parameter]['k'] * DEGREE / 8 + 96 for
                                          security_parameter in ALLOWABLE_SECURITY_PARAMETERS}
ENCODED_CCA_KEM_PK_LEN: dict[int, int] = deepcopy(ENCODED_CPA_PKE_PK_LEN)
ENCODED_CCA_KEM_CIPHERTEXT_LEN: dict[int, int] = {security_parameter: (PARAMS[security_parameter]['d_u'] *
                                                                       PARAMS[security_parameter]['k'] +
                                                                       PARAMS[security_parameter]['d_v']) * DEGREE / 8
                                                  for security_parameter in ALLOWABLE_SECURITY_PARAMETERS}
...
class PolynomialCoefficientMatrix(PolynomialMatrix):
    ...
    def decompress(self, num_bits: int):
        ...
        raise ValueError(
            f'Cannot compute PolynomialCoefficientMatrix.decompress with num_bits unless num_bits is an integer with num_bits >= 1, but had num_bits={num_bits}.')
...
def decode(x: bytes, num_32_bytes: int) -> PolynomialCoefficientMatrix:
...
def cpa_pke_encrypt(security_parameter: int, pk: bytes, m: bytes, r: bytes) -> bytes:
    if security_parameter in ALLOWABLE_SECURITY_PARAMETERS:
        ...
        tmp_three: PolynomialCoefficientMatrix = PolynomialCoefficientMatrix(vals=tmp_two.inv_ntt(), modulus=MODULUS,
                                                                             degree=DEGREE)
        tmp_three += e_two
        decoded_msg: PolynomialCoefficientMatrix = decode(x=m, num_32_bytes=1)  # PROBLEM
